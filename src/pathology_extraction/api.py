...
def extract_report(request: ExtractionRequest):
    ...
    extraction = build_extractor(
        model_config["name"], model_config["prompt"], model_config["response_format"], request.report_text
    )
    ...
    for field_name in PathologyExtraction.model_fields:
        ...
        if field.value is None or field.evidence is None:
            fields[field_name] = FieldScore(value=None, evidence=None, value_evidence_score=None, evidence_semantic_score=None)
            continue
        ...
        fields[field_name] = FieldScore(
            ...
            value_evidence_score=value_score, evidence_semantic_score=semantic_score,
        )
